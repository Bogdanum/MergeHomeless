...
public class LocalNotification
{
    ...
    public static void EnableRetentionNotification ()
    {
        Debug.Log ("[Local-Notification] Start Register the local retention notification!");

        if (!Contains.IsNotificationOn)
            return;

        var time_day = ((DateTime.Parse ("23:59:59")) - DateTime.Now).TotalSeconds + Contains.time_to_push;

        var string_message = ApplicationLanguage.GetRandomDescriptionNotification ();
        var string_tittle  = ApplicationLanguage.GetRandomTitleNotification ();

        if (string.IsNullOrEmpty (string_tittle) || string.IsNullOrEmpty (string_message))
        {
            return;
        }

        SendNotification (0, TimeSpan.FromSeconds (time_day), string_tittle, string_message, new Color32 (0xff, 0x44, 0x44, 255), true, true, true, "notify_icon_big", "notification_sound");
    
        SendNotification (1, TimeSpan.FromSeconds (time_day + 172800), string_tittle, string_message, new Color32 (0xff, 0x44, 0x44, 255), true, true, true, "notify_icon_big", "notification_sound");

    }

    public static void DisableRetentionNotification ()
    {
